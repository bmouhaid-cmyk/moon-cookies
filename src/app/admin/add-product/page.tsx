  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || "Une erreur est survenue lors de l'ajout du produit.");
      }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-serif text-choco">Ajouter un produit</h1>
          <p className="text-choco/60 mt-1">Créez un nouveau produit pour votre boutique</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        {success && (
          <div className="bg-green-50 text-green-600 p-4 rounded-xl flex items-center gap-3 border border-green-100">
            Produit ajouté avec succès ! La mise à jour sur GitHub a été déclenchée.
          </div>
        )}
